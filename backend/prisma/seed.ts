...
import { baseUsername, generateUniqueUsername } from '../src/common/utils/username-generator.util';
...
async function main() {
  ...
  for (const u of usersWithoutUsername) {
    ...
    const newUsername = await generateUniqueUsername(base, async (candidate) => {
      const exists = await prisma.user.findUnique({
        where: { username: candidate },
        select: { id: true },
      });
